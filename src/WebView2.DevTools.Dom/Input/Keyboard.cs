...
namespace WebView2.DevTools.Dom.Input
{
    ...
    public class Keyboard
    {
        ...
        internal Keyboard(DevToolsProtocolHelper client)
        {
            _client = client;
        }
        ...
        public Task DownAsync(string key, DownOptions options = null)
        {
            ...
            var type = text == null ? "rawKeyDown" : "keyDown";
            var isKeyPad = description.Location == 3;

            return _client.Input.DispatchKeyEventAsync(
                type: type,
                modifiers: Modifiers,
        ...
        public Task SendCharacterAsync(string charText)
            => _client.Input.InsertTextAsync(charText);
        ...
        public async Task TypeAsync(string text, TypeOptions options = null)
        {
            ...
            while (textParts.MoveNext())
            {
                ...
                {
                    await PressAsync(letter.ToString(), new PressOptions { Delay = delay }).ConfigureAwait(true);
                }
                else
                {
                    if (delay > 0)
                    ...
                    await SendCharacterAsync(letter.ToString()).ConfigureAwait(true);
                }
            }
        }
        ...
        /// Modifier keys DO effect <see cref="HtmlElement.PressAsync(string, PressOptions)"/>. Holding down <c>Shift</c> will type the text in upper case.
        /// </remarks>
        /// <returns>Task</returns>
        public async Task PressAsync(string key, PressOptions options = null)
        {
            await DownAsync(key, options).ConfigureAwait(true);
            if (options?.Delay > 0)
            {
                await Task.Delay((int)options.Delay).ConfigureAwait(true);
            }
            await UpAsync(key).ConfigureAwait(true);
        }
